...
class Seq2SeqModel(object):
    ...
    def __init__(self, source_vocab_size, target_vocab_size, buckets, size,
        ...
        Args:
            ...
            buckets:                    エンコーダとデコーダの文章の最大文字数のタプルの配列。
                                        e.g. buckets = [(5,10),(10,15),(15,20),...]
            size:                       モデルの各層のユニット数
            num_layers:                 モデルの層の数
            max_gradient_norm:          勾配は、このnormを最大限にクリップする
            batch_size:                 学習中に使用されるバッチサイズ。
                                        モデル構築はbatch_sizeに依存しないため、
                                        デコードなどのために初期化後にbatch_sizeを変更できる
            learning_rate:              学習開始時の学習率。
            learning_rate_decay_factor: 過学習であると判断した場合、learning_rateからこの値を引く。
    ...
    def step(self, session, encoder_inputs, decoder_inputs, target_weights,
        ...
        Returns:
            3つの戻り値（gradient_norm, avg_perplexity, outputs
        Raises:
            ValueError: バケツサイズとencoder_inputs,decoder_inputs,target_weightsが一致していないとき。
        """
        ...
        if not forward_only:
            output_feed = [self.updates[bucket_id],
                           ...
                           self.train_loss_summaries[bucket_id]]
        else:
            if beam_search:
                ...
                output_feed = [self.losses[bucket_id],
                               self.forward_only_loss_summaries[bucket_id]]
        ...
        if not forward_only:
            ...
            if beam_search:
                ...
                return None, outputs[0], outputs[1], outputs[2:]
    ...
    def get_batch(self, data, bucket_id):
        ...
        for _ in range(self.batch_size):
            ...
            # デコーダの入力には余計に"GO"シンボルがついており、それからPADが埋め込まれる
        ...
        # バッチエンコーダの入力は再インデックスしたdecoder_inputsであり、重みを作成する。
        for length_idx in range(decoder_size):
            batch_decoder_inputs.append(
                np.array([decoder_inputs[batch_idx][length_idx]
                          for batch_idx in range(self.batch_size)], dtype=np.int32))

            # パディングするターゲットのtarget_weightsを0にする
